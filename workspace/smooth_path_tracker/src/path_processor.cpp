...
std::vector<TimedPose> PathProcessor::generate_timed_path(const std::vector<Point>& waypoints, double points_per_meter, double desired_velocity) {
    std::vector<Point> smooth_points = generate_smooth_path(waypoints, points_per_meter);
...
std::vector<Point> PathProcessor::generate_smooth_path(const std::vector<Point>& waypoints, double points_per_meter) {
    ...
    for (size_t i = 0; i < waypoints.size() - 1; ++i) {
        ...
        int num_points_per_segment = std::max(2, static_cast<int>(segment_length * points_per_meter));
...
Point PathProcessor::cubic_bezier(Point p0, Point p1, Point p2, Point p3, double t) {
    Point p;
    double u = 1.0 - t;
    double tt = t * t;
    double uu = u * u;
...
void PathProcessor::calculate_control_points(const std::vector<Point>& waypoints, std::vector<Point>& p1_points, std::vector<Point>& p2_points) {
    ...
    if (n == 1) {
        p1_points[0] = {(2 * waypoints[0].x + waypoints[1].x) / 3.0, (2 * waypoints[0].y + waypoints[1].y) / 3.0};
        p2_points[0] = {(waypoints[0].x + 2 * waypoints[1].x) / 3.0, (waypoints[0].y + 2 * waypoints[1].y) / 3.0};
        return;
    }
    ...
    b[0] = 2;
    c[0] = 1;
    r[0] = {waypoints[0].x + 2 * waypoints[1].x, waypoints[0].y + 2 * waypoints[1].y};

    for (int i = 1; i < n - 1; ++i) {
        a[i] = 1;
        b[i] = 4;
        c[i] = 1;
        r[i] = {4 * waypoints[i].x + 2 * waypoints[i+1].x, 4 * waypoints[i].y + 2 * waypoints[i+1].y};
    }

    a[n-1] = 2;
    b[n-1] = 7;
    c[n-1] = 0;
    r[n-1] = {8 * waypoints[n-1].x + waypoints[n].x, 8 * waypoints[n-1].y + waypoints[n].y};
    ...
    for (int i = n - 2; i >= 0; --i) {
        p1_points[i] = {(r[i].x - c[i] * p1_points[i+1].x) / b[i], (r[i].y - c[i] * p1_points[i+1].y) / b[i]};
    }

    for (int i = 0; i < n - 1; ++i) {
        p2_points[i] = {2 * waypoints[i+1].x - p1_points[i+1].x, 2 * waypoints[i+1].y - p1_points[i+1].y};
    }
    p2_points[n-1] = {(waypoints[n].x + p1_points[n-1].x) / 2.0, (waypoints[n].y + p1_points[n-1].y) / 2.0};
}
